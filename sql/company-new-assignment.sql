...
select trim(emp_name),ltrim(emp_name),rtrim(emp_name) from employees;



select concat(emp_name,' ',salary) as name_salary from employees;

select substring(emp_name from 1 for 3) from employees;


select position('a' in emp_name) from employees;

select replace(emp_name,'ahmed','ahmad') from employees;
...
select d.dept_name, count(e.emp_id) from departments d
join employees e
...
update projects set hours_worked = 120 where project_id = 1;

update projects set hours_worked = 90 where project_id = 2;
